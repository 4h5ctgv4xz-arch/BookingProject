...
class APIClient:
    ...
    def get(self, endpoint: str, params=None, status_code=200):
    ...
    def post(self, endpoint: str, data=None, status_code=200):
        url = f"{self.base_url}{endpoint}"
    ...
    def ping(self):
        ...
        with allure.step("Assert status code"):
            assert response.status_code == 201, f"Expected status code of 201, got {response.status_code}"
    ...
    def auth(self):
        with allure.step("Auth API Client"):
            ...
            response.raise_for_status()
        with allure.step("Assert status code"):
            assert response.status_code == 200, f"Expected status code of 200, got {response.status_code}"
        token = response.json()["token"]
        with allure.step("Updating headers with auth token"):
